...
Partendo dall'ipotesi che il diodo sia spento, avremo che $ I = 0$, quindi {\color{code-red}$V_u = 0$}, per $V_u = R\cdot I $.
Il modello inoltre richiede che $V_d < V_\gamma$, e, utilizzando la prima equazione di Kirchoff alla maglia,
otteniamo $V_d = V_i - V_u = V_i$. Quindi il diodo è spento per {\color{code-red}$V_i < V_\gamma$}.
...
\[
    \begin{cases}
        ...
        I \cdot R > 0\\
        V_u = V_i - V_d
    \end{cases} \quad \Rightarrow \quad V_i - V_d > 0 \quad \Rightarrow {\color{code-keyword}{V_i > V_d}}
\]
...
\begin{figure}[h]
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \begin{tikzpicture}
            \begin{axis}[xlabel=$V_i$, ylabel=$V_u$, ymin=-1, width={.9\textwidth}]
                \addplot[code-red, domain=-50:0.78]{0};
                \addplot[code-keyword, domain=0.78:50]{x - 0.78};
    ...
    \begin{minipage}[b]{.48\textwidth}
        \begin{tikzpicture}[
            declare function={
                func(\x) = (\x < 0.78) * 0 + (\x > 0.78) * (\x - 0.78);
            }]
            \begin{axis}[xlabel=$V_i$, ylabel=$V_u$, ymin=-1, width={.9\textwidth}]
                \addplot[code-keyword, samples=30]{func(3*sin(deg(x)))};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
\end{figure}
...
\begin{tcolorbox}[title=Diodo OFF]
    \begin{align*}
        &I = 0 \qquad&\Rightarrow &V_R = 0
        \\
        &V_i - \cancel{V_R} - V_u = 0 \qquad &\Rightarrow & {\color{code-red}V_u = V_i}
        \\
        &V_u < V_\gamma \qquad&\Rightarrow &{\color{code-red}V_i < V_\gamma}
    \end{align*}
\end{tcolorbox}
\end{minipage}
\begin{minipage}[b]{.45\textwidth}
\begin{tcolorbox}[title=Diodo ON]
    \begin{align*}
        &V_d = V_\gamma \qquad&\Rightarrow &{\color{code-keyword}V_u = V_\gamma}
        \\
        &I\cdot R > 0 \qquad&\Rightarrow & V_i - V_d > 0
        \\
        &&& {\color{code-keyword} V_i > V_\gamma}
    \end{align*}
\end{tcolorbox}
\end{minipage}
...
\begin{figure}[h]
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \begin{tikzpicture}
            \begin{axis}[xlabel=$V_i$, ylabel=$V_u$, ymax=2, ymin=-2.5, width={.9\textwidth}]
                \addplot[code-red, domain=-5:0.78]{x};
                \addplot[code-keyword, domain=0.78:5]{0.78};
                \addplot[code-comment, dashed] coordinates {(0.78, -4) (0.78, 1)};
    ...
    \begin{minipage}[b]{.48\textwidth}
        \begin{tikzpicture}[
            ...
            \begin{axis}[xlabel=$V_i$, ylabel=$V_u$, ymax=2, ymin=-2.5, width={.9\textwidth}]
                \addplot[code-keyword, samples=30]{func(2*sin(deg(x)))};
                \addplot [code-comment, dashed]{0.78};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
\end{figure}
...
\begin{tcolorbox}[title=Diodo ON]
    \[
        ...
        \Rightarrow
        \begin{cases}
            {\color{code-red}V_u = V_b - V_\gamma}
            \\
            V_i < V_b - V_\gamma
        \end{cases}
    \]
    ...
    \begin{minipage}[b]{.48\textwidth}
        \begin{tikzpicture}
            \begin{axis}[ymin=-1, ymax=4, width={.9\textwidth}]
                \addplot [code-keyword, domain=-5:\vg]{\vg};
                \addplot [code-red, domain=\vg:5]{x};
                \addplot [code-comment, dashed] coordinates {(\vg, -5) (\vg, 5)};
    ...
    \begin{minipage}[b]{.48\textwidth}
        \begin{tikzpicture}[
            ...
            \begin{axis}[xlabel=$V_i$, ylabel=$V_u$, ymin=-1, ymax=4, width={.9\textwidth}]
                \addplot[code-keyword, samples=30]{func(4*sin(deg(x)))};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
\end{figure}
    ...
    \begin{minipage}[b]{.45\textwidth}
        \begin{tikzpicture}
            \begin{axis}[ymin=-2, ymax=4, width=.9\textwidth]
                ...
                \addplot[dashed, code-comment] coordinates {(2*\vgamma, 5) (2*\vgamma, -5)};
                \addplot[dashed, code-comment] coordinates {(-2*\vgamma, 5) (-2*\vgamma, -5)};
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \begin{axis}[xmin=-1, xmax=10]
            \addplot [code-red, samples=40, domain=-1:10] {vi(deg(x))};
            \addplot [code-keyword, samples=100, domain=-1:10] {func(mod(deg(x) + 360, 360))};
        \end{axis}
    \end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{minipage}{.45\textwidth}
        \begin{circuitikz}
            ...
            \draw[code-keyword]
                (-0.5, 2) -- ++(0, 1.5)
                to[polar capacitor, color=code-keyword, i=$I_C$] ++(2, 0)
                -- ++(0, -1.5)
                ;
        \end{circuitikz}
    \end{minipage}
    \begin{minipage}{.45\textwidth}
        \begin{tikzpicture}
            \begin{axis}[ylabel=$V$, xlabel=$t$, ymin=-4, ymax=4, xmin=-4, xmax=4]
                \addplot [code-keyword] coordinates {(-5, 2) (0, 2)};
                \addplot [code-keyword] coordinates {(0, -2) (5, -2)};
                \addplot [code-comment] coordinates {(-5, 2-\vgamma) (0, 2-\vgamma)};
                \addplot [code-comment] coordinates {(0, 0) (5, 0)};

                \addplot [code-red] coordinates {(3, -2) (3, 2)};
                \addplot [code-red] coordinates {(3, 2) (5, 2)};

                \node [right, code-comment] at (0, 2-\vgamma) {$V_u$};
                \node [right, code-keyword] at (0, 2) {$V_i$};
                \node [above] at (-2, 2) {ON};
    ...
    \begin{minipage}{.32\textwidth}
        \begin{tikzpicture}
            \begin{axis}[width=.95\textwidth, ylabel=$V_u$, xlabel=$t$, xmin=-2, xmax=7]
                \addplot [code-keyword] coordinates {(-5, 3) (0, 3)};
                \addplot [code-keyword] coordinates {(0, -3) (\tm, -3)};
                \addplot [code-keyword] coordinates {(\tm, -3) (\tm, 0)};
                \addplot [code-keyword] coordinates {(\tm, 0) (8, 0)};
                \addplot [code-red] coordinates {(\tm + 2, 0) (\tm + 2, 3)};
                \addplot [code-red] coordinates {(\tm + 2, 3) (8, 3)};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
    ...
    \begin{minipage}{.32\textwidth}
        \begin{tikzpicture}
            \begin{axis}[width=.95\textwidth, ylabel=$Q$, xlabel=$t$, xmin=-2, xmax=8]
                \addplot [code-keyword] coordinates {(-5, 3) (0, 3)};
                \addplot [code-keyword, domain=0:\tm]{(3 / (1-e^(-\tm)))*e ^(-x) - (3 / (1-e^(-\tm))) +3};
                \addplot [code-keyword] coordinates {(\tm, 0) (8, 0)};
                \addplot [code-red, domain=\tm+2:8] {-e^(-x + ln(3) + \tm+2) + 3};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
\end{center}
