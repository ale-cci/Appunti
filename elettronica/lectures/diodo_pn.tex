...
\begin{figure}[h]
    ...
    \begin{minipage}{.45\textwidth}
        \begin{circuitikz}
            ...
            \draw[blue]
                (-0.5, 2) -- ++(0, 1.5)
                to[polar capacitor, color=blue, i=$I_C$] ++(2, 0)
                -- ++(0, -1.5)
                ;
        \end{circuitikz}
    \end{minipage}
    \begin{minipage}{.45\textwidth}
        \begin{tikzpicture}
            \begin{axis}[ylabel=$V$, xlabel=$t$, ymin=-4, ymax=4, xmin=-4, xmax=4]
                \addplot [code-red] coordinates {(-5, 2) (0, 2)};
                \addplot [code-red] coordinates {(0, -2) (5, -2)};
                \addplot [code-keyword] coordinates {(-5, 2-\vgamma) (0, 2-\vgamma)};
                \addplot [code-keyword] coordinates {(0, 0) (5, 0)};

                \node [right, code-keyword] at (0, 2-\vgamma) {$V_u$};
                \node [right, code-red] at (0, 2) {$V_i$};
                \node [above] at (-2, 2) {ON};
...
\noindent Per $t < 0$, il circuito è ancora in regione statica, per questo la derivata della corrente è nulla e valgono ancora le equazioni del diodo in
regione statica.
Stesso ragionamento è valido per $t \to \infty$.

\begin{tcolorbox}[title={$t < 0$}]
\end{tcolorbox}

\begin{tikzpicture}
    \begin{axis}
        \addplot coordinates {(-5, 3) (0, 3)};
    \end{axis}
\end{tikzpicture}
