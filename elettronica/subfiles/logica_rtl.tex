...
\begin{tikzpicture}
    ...
    \begin{axis}[ymax=6, ymin=-1, xmin=-1]
        \addplot[thin, red, domain=-5:\vgamma]{5};
        \addplot[thin, red, domain=\vgamma:{(5 - \vcesatval)*\rb/(\betaf*\rc) + \vgamma}]{5 - \betaf*\rc/\rb*(x - \vgamma)};
        \addplot[thin, red, domain={(5 - \vcesatval)*\rb/(\betaf*\rc) + \vgamma}:5]{\vcesatval};

        \addplot[thin, black, domain=-5:\vgamma]{(\rb * 5 + \rc * \vgamma)/(\rb + \rc)};
        \addplot[thin, black, domain=\vgamma:{-(\vgamma - (\rb * 5 + \rc *\vgamma)/(\rb + \rc))*((\rb + \rc)/(\betaf*\rc)) + \vgamma}]{(\rb * 5 + \rc * \vgamma)/(\rb + \rc) - (\betaf*\rc)/(\rb + \rc) * (x - \vgamma)};
        \addplot[thin, black, domain={-(\vgamma - (\rb * 5 + \rc *\vgamma)/(\rb + \rc))*((\rb + \rc)/(\betaf*\rc)) + \vgamma}:{(5 - \vcesatval)*\rb/(\betaf*\rc) + \vgamma}]{5 - \betaf*\rc/\rb*(x - \vgamma)};
