...
È possibile quindi distinguere due regioni di funzionamento: una valida per $V < V_T$ caratterizzata da una bassa corrente,
ed una caratterizzata da una corrente elevata, valida per $V > V_T$. Le due regioni prendono il nome di
\textit{polarizzazione diretta} e \textit{polarizzazione inversa}.
...
Dal modello a soglia utilizzato per la carica, essa è positiva, fino a quando la tensione ai capi della giunzione si
mantiene costante e pari a $V_\gamma$.
...
(Comportamento del modello in blu)
\begin{center}
    \def\tm{3}
